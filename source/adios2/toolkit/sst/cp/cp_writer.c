    ...
    void *FreeAttributelientData)
{
    ...
    ADIOST_START("First MPI operation in EndStep()");
    MPI_Allreduce(&GlobalOpRequested, &GlobalOpRequired, 1, MPI_INT, MPI_LOR,
                  Stream->mpiComm);
    ADIOST_STOP("First MPI operation in EndStep()");
    if (GlobalOpRequired)
    {
        ADIOST_START("Global operations");
        ...
        ADIOST_STOP("Global operations");
    }

    ADIOST_START("provide timestep operations");
    ...
    if (Data)
        ADIOST_SAMPLE_COUNTER("Timestep local data size", Data->DataSize);
    if (LocalMetadata)
        ADIOST_SAMPLE_COUNTER("Timestep local metadata size",
    ...
    ADIOST_START("Metadata Consolidation time in EndStep()");
    if (Stream->ConfigParams->CPCommPattern == SstCPCommPeer)
    {
    ...
    ADIOST_STOP("Metadata Consolidation time in EndStep()");
    ...
    if (Data == NULL)
    {
        ...
        PTHREAD_MUTEX_UNLOCK(&Stream->DataLock);
        ADIOST_STOP("provide timestep operations");
        return;
    }
